...
// This one has a domain of the integers and the range is (within) the integers.
// Again, `int` is the entire set of mathematical integers.

// Run dafny on this file. See where it fails. Fix it.

ghost function Double(val:int) : int
{
  // The body of a function is an expression context. No semicolon at the end.
  2 * val
}
...
// As in C, statements terminate with semicolons and can be grouped into blocks with braces.
